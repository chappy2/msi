...
build = (watch, callback, {useMapping} = {}) ->
  ...
  # when callback is mocha (testing) then skip client building. 
  # tests are only a example for server.
  if not callback.toString().includes('mocha')
    # compile client code in public folder
    options = ['-c']
    options.push("-m") if useMapping
    options.push("-o")
    options.unshift '-w' if watch
    options = options.concat clientFiles
    launch 'coffee', options, callback
...
unlinkIfCoffeeFile = (file) ->
  if file.match /\.coffee$/
    fs.unlink file.replace('src','dist').replace(/\.coffee$/, '.map'), ->
    fs.unlink file.replace('src','dist').replace(/\.coffee$/, '.js'), ->
    true
  else false
...
mocha = (options, callback) ->
  ...
  ###
  mocha options added to test/mocha.opts
  ###
...
docco = (callback) ->
  ...
  walk 'public/src', (err, files) -> launch 'docco', files, callback
